#!/usr/bin/env python3
"""
Batch Processor for JSON Layout to EM Simulation

This script processes multiple JSON layout files in batch mode using the CLI interface.
It supports parallel processing, progress tracking, and comprehensive logging.

Usage:
    python batch_processor.py --config batch_config.json
    python batch_processor.py --json-dir ./layouts --workspace ./batch_results
...
class BatchProcessor:
    """Batch processing engine for EM simulation workflows"""
    ...
    def setup_logging(self):
        ...
        self.logger = logger
        self.logger.info(f"Batch processor started - log file: {log_file}")
    ...
    def get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_workers": 1,  # Sequential processing to avoid ADS conflicts
    ...
    def create_job_config(self, json_file: Path, base_config: Dict[str, Any], job_index: int) -> Dict[str, Any]:
        ...
        job_config = {
            ...
            "retry_count": base_config.get("retry_count", 2)
        }
    ...
    def run_single_job(self, job_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if job_config.get("export_path"):
                # Convert to absolute path to ensure consistent behavior
                cmd.extend(["--export-path", str(export_dir)])
            ...
            result["error"] = str(e)
            result["error_details"] = {"exception": str(e), "traceback": traceback.format_exc()}
    ...
    def run_batch_sequential(self, jobs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Run batch jobs sequentially (recommended for ADS)"""
...
"""
        ...
        for result in results:
            ...
            report += f"- **{status_emoji} {status}**: {result['cell_name']} ({result['duration']:.1f}s)"
            if not result["success"]:
    ...
    def process_files(self, json_files: List[Path], config: Dict[str, Any]) -> bool:
        ...
        # Run jobs sequentially (recommended for ADS)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Batch processor for JSON Layout to EM Simulation",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Process all JSON files in directory
  python batch_processor.py --json-dir ./layouts

  # Process with custom config
  python batch_processor.py --config batch_config.json

  # Process specific pattern
  python batch_processor.py --json-dir ./layouts --pattern "design_*.json"

  # Use PDK and custom settings
  python batch_processor.py --json-dir ./layouts --workspace ./results --library Custom_Lib --use-pdk
        """
    )
