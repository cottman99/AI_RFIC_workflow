#!/usr/bin/env python3
"""
Subprocess Worker CLI for ADS/EMPro Operations
...
Usage: python subprocess_worker_cli.py <task_json>
"""
...
def setup_logging():
    ...
    log_filename = Path(tempfile.gettempdir()) / f"ads_worker_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
def run_em_simulation_task(task_data):
    ...
    try:
        ...
        path_mode = export_config.get('path_mode', 'absolute')
...
class UnitConverter:
    """Handle unit conversions for layout dimensions"""
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python subprocess_worker_cli.py <task_json_file>")
        sys.exit(1)
